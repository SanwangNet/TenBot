...
test("a reply with no valid messages uses one safe error notice", async () => {
    ...
    assert.equal(calls[0].args[1], "ERROR: B:A_RA_IRA");
...
test("two timeout attempts keep the explicit fallback and discard the late AI result", async (t) => {
    ...
    assert.deepEqual(calls.map((call) => call.method), ["text"]);
    assert.equal(calls[0].args[1], MODEL_TIMEOUT_MESSAGE);
...
test("an overdue result is discarded and the retry gets a fresh deadline", async (t) => {
    ...
    await pending;
    assert.deepEqual(signals.map((signal) => signal.aborted), [true, true]);
    assert.deepEqual(calls.map((call) => call.args[1]), [MODEL_TIMEOUT_MESSAGE]);
    t.mock.timers.reset();
});
...
test("confirmed provider 503 and unclassified 520 produce their terminal error codes promptly", async (t) => {
    ...
    for (const [upstreamError, expectedCode] of [
        ...
        assert.deepEqual(calls.map((call) => call.method), ["text"]);
        assert.equal(calls[0].args[1], `ERROR: ${expectedCode}`);
        assert.equal(isConversationActive(trigger), false);
        t.mock.timers.tick(30_001);
        assert.equal(calls.length, 1);
    }
...
test("provider error code fallback keeps the existing trigger quote decision", async () => {
    ...
    assert.equal(payload.markdown.content, "ERROR: M:A_MG_MRF");
...
test("web search notice is followed by the terminal model timeout code", async (t) => {
    ...
    assert.deepEqual(calls.map((call) => call.args[1]), ["稍等，我查一下。", MODEL_TIMEOUT_MESSAGE]);
    t.mock.timers.reset();
});
...
test("unknown fatal AI error gets the generic model request code", async () => {
    ...
    assert.deepEqual(calls.map((call) => call.method), ["text"]);
    assert.equal(calls[0].args[1], "ERROR: M:A_MG_MRF");
});
