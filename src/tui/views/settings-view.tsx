...
export function SettingsView({ status, config, selectedIndex, pendingRestart, viewedProvider, width, registry, onEdit, onViewProvider, onApplyProvider }: {
    ...
    const rows = settingsRows(viewedProvider);
    ...
    const editableRow = (field: SettingsField, index: number, width: number) => <ClickableRegion key={field} id={`settings:${field}`} registry={registry} width="100%" flexShrink={0} onClick={() => onEdit(field)}>
        <Box flexDirection="row">
            ...
            <Text wrap="truncate">{valueFor(field)}</Text>
        </Box>
    </ClickableRegion>;

    const horizontal = width >= 86;
    return <Box flexDirection={horizontal ? "row" : "column"} alignItems="stretch" width="100%" flexGrow={1}>
            <Box flexGrow={horizontal ? 1 : 0} flexBasis={horizontal ? 0 : undefined} flexShrink={1} minWidth={0}>
                <Panel flexGrow={1}>
            ...
            <Box flexGrow={horizontal ? 1 : 0} flexBasis={horizontal ? 0 : undefined} flexShrink={1} minWidth={0}>
                <Panel title="回复判断" flexGrow={1}>
                    {editableRow("replyJudge.model", rows.indexOf("replyJudge.model"), 14)}
                    {editableRow("replyJudge.timeoutMs", rows.indexOf("replyJudge.timeoutMs"), 14)}
                </Panel>
            </Box>
        </Box>;
}
